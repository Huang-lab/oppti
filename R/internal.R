...
#   cbindNA
#
#   bind arbitrary-length columns given in a list into a dataframe with NA-filled ends
#
...
#' @title Putative outlier evaluation
#' @description  Calculate potential outlier scores (p-statistic) as a function of distance from 1st/3rd (box) quartiles. If the value lies within the box the score is 1, if it lies further away from the box the score gets closer to 0.
#' @return an object of the same class as dat containing the potential outlier scores.
outScores = function(dat) {
    n = ncol(dat)
    dat.IQR = matrix(rep(apply(dat, 1, 'IQR', na.rm = TRUE), each = n), ncol = n, byrow = TRUE)
    dat.q75 = matrix(rep(apply(dat, 1, function(x){quantile(x, .75, na.rm = TRUE)}), each = n), ncol = n, byrow = TRUE)
    dat.q25 = matrix(rep(apply(dat, 1, function(x){quantile(x, .25, na.rm = TRUE)}), each = n), ncol = n, byrow = TRUE)
...
#
gqplot = function(y, x, ci = 0.95, xlab = NULL, ylab = NULL, dist.sort = FALSE, d.thr = 0, na.action = 'omit', samp.names = NULL, marker.name = NULL, highlight = NULL, omit.fit = NULL, maxx = NULL, minx = NULL, maxy = NULL, miny = NULL, align.xy = TRUE) {
    # marker name
    ...
    if ((any(is.na(y)) | any(is.na(x))) & na.action == 'omit'){
        ...
        x = x[keep]
        y = y[keep]
        if (!is.null(highlight)) {highlight[highlight %in% rownames(keep)[keep]]}
    }
    # sample names
    if (is.null(samp.names)) {samp.names = names(y)}
    if (is.null(samp.names)) {samp.names = names(x)}
    # align classes
    if (!is(y, 'vector')) {y = as.vector(y)}
    if (!is(x, 'vector')) {x = as.vector(x)}
    # define labels
    ...
    if (is.null(omit.fit)) {
        fit = stats::lm(y ~ x)
        preds = predict(stats::lm(y ~ x, na.action = na.exclude), interval = 'confidence', level = ci)
    } else if (omit.fit < 1) {
        fit = stats::lm(y ~ x)
        preds = predict(stats::lm(y ~ x, na.action = na.exclude), interval = 'confidence', level = ci)
    } else {
        y.s = sort(y, decreasing = TRUE, index.return = TRUE, na.last = TRUE)
        non.out = (y < y[y.s$ix[omit.fit]]) & (y > y[y.s$ix[length(y[!is.na(y)])-omit.fit+1]])
    ...
    # y = slope x + intercept; ax + by + c = 0; by = -ax -c; d_x0_y0 = |ax0 + by0 + c| / sqrt(a^2 + b^2)
    # b = 1; a = -slope; c = -intercept
    d = (y - fit$coefficients[2]*x - fit$coefficients[1]) / sqrt(fit$coefficients[2]**2+1)
    if (dist.sort) {
        sd = sort(d, decreasing = TRUE, index.return = TRUE, na.last = TRUE)
        outlier.score = data.frame(sampleID = samp.names[sd$ix[sd$x >= quantile(sd$x, d.thr, na.rm = TRUE)]], dist2reg = sd$x[sd$x >= quantile(sd$x, d.thr, na.rm = TRUE)])
    } else {
        outlier.score = data.frame(sampleID = samp.names[d >= quantile(d, d.thr, na.rm = TRUE)], dist2reg = d[d >= quantile(d, d.thr, na.rm = TRUE)])
    }
    # when there is NA, preds is shorten, handle NAs: (NA values dropped)
    df = data.frame(variable1 = y, variable2 = x, clower   = preds[,2], cupper   = preds[,3])
    rownames(df) = samp.names
    # newx = seq(min(df$variable2), max(df$variable2), length.out=length(df$variable2))
    ...
    if (is.null(highlight)) {
        gg = ggplot2::ggplot(df, ggplot2::aes(x=variable2, y=variable1)) +
            ggplot2::geom_point(ggplot2::aes(variable2, variable1), shape = 1, size = 3) +
            ggplot2::geom_abline(intercept = fit$coefficients[1], slope = fit$coefficients[2], alpha = 0.5) +
        ...
        gg = ggplot2::ggplot(df, ggplot2::aes(x=variable2, y=variable1)) +
            ...
            ggplot2::ylim(miny,maxy) +
            ggplot2::ggtitle(marker.name) +
            ggplot2::geom_point(data = df[highlight,], ggplot2::aes(x=variable2, y=variable1), colour = 'red')
        # ggplot2::geom_polygon(c(rev(newx), newx), c(rev(preds[,3]), preds[,2]), col = 'grey80', border = NA)
    }
